describe("checking for truth", () => {
  test("are the littleNumbers < bigNumbers?", () => {
    ...
    let littleNumbers: Array<number> = [0, 1, 2, 3, 4, 5];
    let bigNumbers: Array<number> = [6, 7, 8, 9, 10, 11];
  ...
  test("wordToCheck is a palindrome, case and everything", () => {
    let wordToCheck = "NaN"; // yeah, replace the value to pass the test.
  ...
  test("there is a sequence to this test", () => {
    let values: Array<any> = ["cat", true, 8, null ]; // set values here - the array will take any type or combo of types.
